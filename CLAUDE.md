...
**Current version: v3.0.3** (v3.0.0 was tagged "Final release"; v3.0.1–v3.0.3 are follow-up fix rounds — see `CHANGELOG.md` for full history). Live demo: https://weatherview-sol5.onrender.com. Repo: `git@github.com:yashmore18/WeatherView.git` (origin), default branch `main`.
...
- **Translucent "glass" surfaces over the animated scene need two different opacity levels, not one** — `--wv-glass-text-scrim-light/dark` (~62% opaque) is right for `.glass-card`/`.wv-header`/sidebar, which sit over the scene itself and are meant to still hint at it. Any `.glass-dropdown` (search/autocomplete suggestions) sits over genuinely arbitrary page content instead (map tiles, hero cards, other cards) and needs the separate, near-opaque `--wv-dropdown-scrim-light/dark` (~96%, added in v3.0.3) — at 62% the *text* stayed readable but the dropdown's own background still visibly changed shade row-to-row depending what was behind each option, which reads as "the dropdown is still overlapping" even when z-index/stacking is provably correct (verify with `document.elementFromPoint`, not just by eyeballing which element "looks on top"). If a future glass surface is reported as "bleeding through" again, check which of these two tokens it's using before assuming a stacking bug.
...
```
Existing tags: `v1.0.0`, `v2.0.0`, `v3.0.0`, `v3.0.1`, `v3.0.2`, `v3.0.3` (`git tag -l -n1` to see summaries). Don't push without the user's go-ahead if it wasn't already explicitly requested for this round of changes.
